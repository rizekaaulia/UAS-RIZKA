   ...
   //pengujian apakah data akan di edit atau simpan baru
	if ($_GET['hal']=="edit") {
		...
		if($edit)// jika edit data sukses
		{
			...
			document.location='tambahcategory.php';
			</script>";
...
if (isset($_GET['hal'])) {
	...
	if ($_GET['hal']=="edit") {
		...
		if ($data) {
			...
			$vkb=$data['categoty_id'];
			$vkp=$data['category_name'];
		}
	}
	elseif($_GET['hal']=="hapus"){
		//persiapan hapus data
		$hapus=mysqli_query($koneksi, "DELETE FROM tb_categoty WHERE id='$_GET[id]'");
		if ($hapus) {
			echo "<script>
...
<body >
	...
	<div class="card-body">
		<form method="POST" action="">
			...
			<button type="Submit" class="btn" name="buttonsimpan"> Simpan </button>
